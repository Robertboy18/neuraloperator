    ...
    do
        ngc batch run \
            ...
            --commandline "bash -c '\
                ...
                python train_navier_stokes.py --opt.mode="exp" --opt.base_lr=$BASE_LR --opt.max_lr=$MAX_LR --incremental.incremental_resolution.use = True;\
            '"
    done
done
        ...
        do
            ngc batch run \
                ...
                --commandline "bash -c '\
                    ...
                    python train_navier_stokes.py --opt.mode="exp" --opt.base_lr=$BASE_LR --opt.max_lr=$MAX_LR --incremental.incremental_resolution.use=True --incremental.incremental_grad.use=True --incremental.incremental_grad.grad_explained_ratio_threshold=$THRESHOLD;\
                '"
        done
    done
done
        ...
        do
            ngc batch run \
                ...
                --commandline "bash -c '\
                    ...
                    git checkout robert-test-incremental; \
                    cp -r /ngc_workspace/jiawei/projects/ifno/data /workspace/fly-incremental/data; \
                    python train_navier_stokes.py --opt.mode="triangular" --opt.base_lr=$BASE_LR --opt.max_lr=$MAX_LR --incremental.incremental_resolution.use=True --incremental.loss_gap.use=True --incremental.loss_gap.eps=$EPS;\
                    python train_navier_stokes.py --opt.mode="triangular2" --opt.base_lr=$BASE_LR --opt.max_lr=$MAX_LR --incremental.incremental_resolution.use=True --incremental.loss_gap.use=True --incremental.loss_gap.eps=$EPS;\
                    python train_navier_stokes.py --opt.mode="exp" --opt.base_lr=$BASE_LR --opt.max_lr=$MAX_LR --incremental.incremental_resolution.use=True --incremental.loss_gap.use=True --incremental.loss_gap.eps=$EPS;\
                '"
        done
    done
done