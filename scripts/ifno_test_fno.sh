        ...
        do
            ngc batch run \
                ...
                --commandline "bash -c '\
                    ...
                    python train_2d.py --opt.scheduler="StepLR" --opt.learning_rate=$BASE_LR --checkpoint.name="checkpoints34" --incremental.incremental_resolution.use=True --opt.weight_decay=$EPS --incremental.incremental_resolution.epoch_gap=$EPOCH;\
                '"     
        done    
    done
done
        ...
        do
            ngc batch run \
                ...
                --commandline "bash -c '\
                    ...
                    git checkout robert-turbulence; \
                    cp -r /ngc_workspace/jiawei/projects/ifno/data /workspace/fly-incremental/data; \
                    python train_2d.py --opt.scheduler="StepLR" --opt.learning_rate=$BASE_LR --checkpoint.name="checkpoints33" --opt.weight_decay=$EPS;\
                '"     
        done    
    done
done