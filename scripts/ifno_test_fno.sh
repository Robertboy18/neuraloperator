    ...
    do
        ngc batch run \
            ...
            --commandline "bash -c '\
                ...
                python train_navier_stokes.py --opt.scheduler="StepLR" --opt.learning_rate=$BASE_LR --checkpoint.name="checkpoints31" --incremental.incremental_loss_gap.use=True --incremental.incremental_loss_gap.eps=$EPS;\
            '"
    done
done

TASK_NAME=ifno_batch_script_test_fno_turbulence2

for BASE_LR in 1e-3
do
    for EPS in 0.999 0.99
    do
        ngc batch run \
            --name "ml-model.$TASK_NAME" \
            --preempt RUNONCE \
            ...
            --commandline "bash -c '\
                ...
                git checkout robert-turbulence; \
                cp -r /ngc_workspace/jiawei/projects/ifno/data /workspace/fly-incremental/data; \
                python train_navier_stokes.py --opt.scheduler="StepLR" --opt.learning_rate=$BASE_LR --checkpoint.name="checkpoints32" --incremental.incremental_grad.use=True --incremental.incremental_grad.grad_explained_ratio_threshold=$EPS;\
            '"
    done
done
