...
L_array = np.random.uniform(low=1,high=20,size=(2,))*mm

# (2) Poling period mismatch can be varied from ~-50 nm to 50 nm
#pp_mismatch_array = np.linspace(-50,50,10)*nm
pp_mismatch_array = np.random.uniform(low=-50.0,high=50,size=(2,))*nm
...
pump_energy_array = np.random.uniform(low=5,high=5000,size=(2,))*fJ
...
def shg_iters(data,pp_mismatch_array,L_array,pump_energy_array,wg,t,f_ref,f2,kk,jj,qq):
    ...
    print("Hello4")
    print("LHS_Shape", data[kk*len(pp_mismatch_array)*len(pump_energy_array)+jj*len(pump_energy_array)+qq,(nt+4):].shape)
    print("RHS_Shape", output_field.shape)
    data[kk*len(pp_mismatch_array)*len(pump_energy_array)+jj*len(pump_energy_array)+qq,(nt+4):] = output_field;
    print("Hello5")
    
 
    
    # save the data every 250 iterations
    if (kk*len(pp_mismatch_array)*len(pump_energy_array)+jj*len(pump_energy_array)+qq)%250 == 0:
        
        print("In saving for loop")
        
        print(f"Saved data for L = {L/mm} mm, pp_mismatch = {pp_mismatch/nm} nm, pump_energy = {E2/fJ} fJ")
        ...
        abc = kk*len(pp_mismatch_array)*len(pump_energy_array)+jj*len(pump_energy_array)+qq
        # Save the DataFrame to a CSV file
        csv_file_path = f'/home/vduruiss/Val/TEMP_Jobs_EM/data/emSHG_output_{abc}.csv'
...
# Save the DataFrame to a CSV file
csv_file_path = '/home/vduruiss/Val/TEMP_Jobs_EM/data/emSHG_output_temp_new.csv'
df.to_csv(csv_file_path, index=False)
