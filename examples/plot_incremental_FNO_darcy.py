...
ranks = [0.01, 0.1, 0.2, 0.5, 0.75, 0]
update_proj_gap = [1, 5, 10, 20]
lr = [1e-1, 1e-2, 1e-3]
for k in lr:
    for j in update_proj_gap:
        for i in ranks:
            ...
            if i != 0:
                wandb_name = f"tensor_incremental_fno_darcy_rank_{i}"
            else:
                wandb_name = f"tensor_fno_darcy_baseline"
            wandb_args =  dict(
                name=wandb_name,
                group='',
                project='darcy',
            ...
            if i != 0:
                ...
                param_groups = [{'params': regular_params}, 
                                {'params': galore_params, 'rank': i , 'update_proj_gap': j, 'scale': 0.25, 'proj_type': "std"}]
                param_groups1 = [{'rank': i , 'update_proj_gap': j, 'scale': 0.25, 'proj_type': "std"}]
                optimizer = AdamW(param_groups, lr=k)
