...
from utils.griddataset import MixedTemporalDataset, MixedTemporalDatasetCodano, SingleDatasetSampler
from utils.make_master_file import DATASET_DICT
from models.fno import FNO2d
from models.dpot import DPOTNet
from models.dpot_res import CDPOTNet
from models.codafno import CoDAFNONet, CoDANO, Super_VSPE
...
# CUDA_VISIBLE_DEVICES="0,1" accelerate launch --num_processes 2 --multi_gpu train_temporal_parallel-main.py
################################################################
...
def get_args():
    ...
    parser.add_argument('--ntrain_list', nargs='+', type=int, default=[10])
...
if __name__ == "__main__":
    ...
    train_dataset = MixedTemporalDatasetCodano(args.train_paths, args.ntrain_list, res=args.res, t_in = args.T_in, t_ar = args.T_ar, normalize=False,train=True,data_weights=args.data_weights)
    test_datasets = [MixedTemporalDatasetCodano(test_path, res=args.res, n_channels = train_dataset.n_channels,t_in = args.T_in, t_ar=-1, normalize=False, train=False) for test_path in test_paths]
    data_sampler = SingleDatasetSampler(train_dataset, batch_size=args.batch_size, shuffle=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, num_workers=12, batch_sampler=data_sampler)
    ...
    # load model
    ################################################################
    model = CoDANO(img_size=args.res, in_channels=train_dataset.n_channels, in_timesteps = args.T_in, out_timesteps = args.T_bundle, out_channels=train_dataset.n_channels, normalize=args.normalize, embed_dim=args.width, modes=args.modes, depth=args.n_layers, n_blocks = args.n_blocks, mlp_ratio=args.mlp_ratio, act=args.act, n_cls=len(args.train_paths)).to(device)
    vspe_model = Super_VSPE(data_names=args.train_paths, n_channels=None, embed_dim=args.width, modes1=args.modes, image_size=args.res).to(device)
    ...
    log_path = '/pscratch/sd/r/rgeorge/weights_codano/' + time.strftime('%m%d_%H_%M_%S') + comment if len(args.log_path)==0  else os.path.join('/pscratch/sd/r/rgeorge/weights_codano/logs_pretrain',args.log_path + comment)
    model_path_fun = lambda epoch: (log_path + '/model_{}.pth'.format(epoch))
    model_path_fun1 = lambda epoch: (log_path + '/vspe_model_{}.pth'.format(epoch))
    ckpt_save_epochs = 5
    if args.use_writer:
    ...
    for ep in range(args.epochs):
        ...
        for xx, yy, msk, cls, n_variables in train_loader:
            ...
            msk = msk.to(device)
            cls = cls.to(device)
            n_variables = n_variables.to(device)
            print('xx',xx.shape, 'yy',yy.shape, 'msk',msk.shape, 'cls',cls.shape, cls[0], 'n_variables',n_variables.shape)
            ...
            for t in range(0, yy.shape[-2], args.T_bundle):
                ...
                vspe1 = vspe_model(xx, pde_type=str(cls[0].item()))
                #print('vspe1',vspe1.shape, 'n_variables',n_variables[1].item(), 'cls',cls.squeeze()[0].item())n_variables[1].item()
                im, cls_pred = model(xx, n_variables=n_variables[1].item(), vspe=vspe1)
        ...
        with torch.no_grad():
            ...
            for id, test_loader in enumerate(test_loaders):
                ...
                for xx, yy, msk, cls, n_variables in test_loader:
                    loss = 0
                    xx = xx.to(device)
                    yy = yy.to(device)
                    msk = msk.to(device)
                    ...
                    for t in range(0, yy.shape[-2], args.T_bundle):
                        y = yy[..., t:t + args.T_bundle, :]
                        vspe1 = vspe_model(xx, pde_type=str(cls[0].item()))
                        im, cls_pred = model(xx, n_variables[1].item(), vspe=vspe1)
                        loss += myloss(im, y, mask=msk)
        ...
        if args.use_writer:
            ...
            torch.save({'args': args, 'vspe_model': vspe_model.state_dict(), 'optimizer': optimizer.state_dict()}, model_path_fun1(ep // ckpt_save_epochs))
