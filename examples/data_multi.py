...
pump_energy_array = np.linspace(5,10000, 20)*fJ
...
def shg_iters(data,pp_mismatch_array,L_array,pump_energy_array,wg,t,f_ref,f2,kk,jj,qq):
    ...
    if (kk*len(pp_mismatch_array)*len(pump_energy_array)+jj*len(pump_energy_array)+qq)%250 == 0:
        ...
        csv_file_path = f'/raid/robert/emSHG_output_{abc}.csv'
...
# Save the DataFrame to a CSV file
csv_file_path = '/raid/robert/emSHG_output.csv'
df.to_csv(csv_file_path, index=False)
